...
export default function Skills() {
  return (
    <section id="skills" className="py-32 paper-bg border-t border-ink/10">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-12 gap-12 mb-16 reveal">
          <div className="md:col-span-4">
            <div className="text-xs uppercase tracking-[0.3em] text-neutral-600 mb-4">Stack</div>
            <div className="font-serif-display text-5xl text-ink">Tools I use to <span className="italic-serif">ship.</span></div>
          </div>
        </div>
        ...
        <div className="space-y-12 reveal">
          {categories.map((cat) => (
            <div key={cat.n} className="grid grid-cols-[60px_1fr] md:grid-cols-[120px_1fr] gap-6 items-start">
              <div className="pt-1">
                <div className="text-xs font-mono text-neutral-500">{cat.n}</div>
              </div>
              <div>
                <div className="text-sm uppercase tracking-widest text-ink mb-4 font-medium">{cat.title}</div>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                  {cat.items.map((item) => (
                    <div key={item.name} className="bg-bone border border-ink/10 rounded-lg p-4 flex flex-col items-center justify-center gap-2 hover:border-ink/30 transition aspect-square relative">
                      {item.svg ? (
                      ...
                      <span className={`text-xs ${item.learning ? 'italic-serif text-ink' : 'text-neutral-700'}`}>{item.name}</span>
                      {item.learning && (
                        <span className="absolute top-1.5 right-1.5 text-[9px] font-mono text-neutral-500 border border-ink/20 px-1.5 py-0.5 rounded">
                          learning
                        </span>
                      )}
                    </div>
